...
- If the target user is suspended, the endpoint returns `403` with `suspended: true` and their `username`, and no profile fields. This is checked before the block check.
...
A suspended user returns `403` with no profile fields.

```json
{
  "error": "This user has been suspended indefinitely for violating our Terms of Service.",
  "suspended": true,
  "username": "playertwo"
}
```