...
- **Album with one surviving file:** when an album request ends with exactly one stored file, no album is created and the response uses the individual shape (`post.posts` with one entry).
...
`priority` is `true` when the upload is a video queued for priority (adaptive) streaming, and `false` otherwise. Priority minutes are charged after the response; if your budget runs out first, the video falls back to standard quality and [`GET /posts/{code}`](/api-reference/endpoint/post) reports `priority: false`.
