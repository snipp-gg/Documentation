...
The bot accepts images (`.png`, `.jpg`, `.webp`, `.gif`) for every plan. Video uploads through the bot are a **Plus and Enterprise** feature and are capped at **100 MB** per file (Discord's own attachment limit). Free accounts can still upload videos from the [web uploader](https://snipp.gg/upload) or the [`/upload` endpoint](/api-reference/endpoint/upload).
