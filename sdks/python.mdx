...
```python
client = SnippClient(api_key="YOUR_API_KEY", region="eu-west-1")  # or "us-west-1"
```
...
Fetch a user by ID. Pass `"@me"` or omit it for the authenticated user.

| Parameter | Type | Description |
|-----------|------|-------------|
| `user_id` | `str` | User ID or `"@me"`. Defaults to `"@me"`. |
...
| `privacy` | `str \| None` | `"public"`, `"unlisted"`, or `"private"`. Defaults to `private` when omitted. |
| `title` | `str \| None` | Optional post title (max 30 chars). |
| `description` | `str \| None` | Optional post description (max 200 chars). |
| `post_type` | `str \| None` | Sent as the `post-type` header. Has no effect through `upload()`, which sends a single file; use `append_upload` to build an album. |
