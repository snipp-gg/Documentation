...
Append 1 or more files to an existing album post. The post's share code, privacy, title, and description are preserved. Albums cap at 9 files total; requests that would exceed the cap are rejected. New files inherit the post's privacy — returned URLs are signed with a 24-hour expiry for private posts.

| Parameter | Type | Description |
|-----------|------|-------------|
| `code` | `string` | Share code of the post to append to. |
| `files` | `Array<File \| Blob \| Buffer \| Uint8Array>` | 1 or more files to append. |
