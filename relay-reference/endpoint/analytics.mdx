...
Fetch your own analytics: posts, uploads, data uploaded, comments received and likes received over time, plus lifetime totals. This is the same data that powers the Analytics page in the Snipp Console, exposed so you can rebuild it in your own tools.
...
- `payout` is the creator payout estimate for the current calendar month, for partners only. `payout.eligible` is `false` for non-partners; when `true`, `payout.payout` is the estimated dollar share and `payout.series` is the daily score for the month. The estimate is live and may change as engagement changes through the month.
- All dates are UTC.
