...
- `postThumb` is null for videos with no thumbnail. `postThumb` and `postText` are both null for posts that have been moderated.
- If the related post is no longer visible to you (it was made private, is a draft, or belongs to a team you are not in), `postThumb`, `postText` and `body` are all null. `body` is also null when every actor on the entry has been suspended.
