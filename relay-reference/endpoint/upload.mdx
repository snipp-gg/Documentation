...
| `include-metadata` | No | `true` to keep EXIF metadata. Omitted or `false` strips it (default). |
...
- **`post-privacy: private`:** the returned `url` (and any `urls` in albums) is a signed URL with a 24-hour expiry. After it expires, call [`GET /posts/{code}`](/relay-reference/endpoint/post) or [`GET /uploads`](/relay-reference/endpoint/uploads) to get a fresh signed URL.
...
- **Metadata:** EXIF metadata (camera, GPS location, timestamp) is stripped from PNG, JPG, and WebP uploads by default. Send `include-metadata: true` to keep it. Stripping applies to images up to 100 MB; larger images are stored unchanged.
