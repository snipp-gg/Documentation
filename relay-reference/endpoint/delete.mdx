...
- Get the filename from the `files[].fileName` field on [`GET /posts/{code}`](/relay-reference/endpoint/post), or from the last segment of `url` on [`GET /uploads`](/relay-reference/endpoint/uploads).
...
- Files on a moderated post cannot be deleted. The request returns `403`.
